...
function TriggerHandler:onUpdate(dt)
	...
	if next(self.triggers) == nil and (self.validFillTypeLoading or self.driver:getIsInFilltrigger()) then 
...
--used for checking if we are in unloadingTriggers 
...
--[[	if not self:isStopped() then 
		...
		elseif next(self.objectsInTrigger) == nil then
			self.objectsInTrigger = {}
		end
	end
	]]--
